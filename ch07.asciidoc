...
However, to spend the Segregated Witness output, the transaction has no signature on that input. Instead, Bob’s transaction has an empty +scriptSig+ in the transaction data (the first part of a transaction) and includes his signature in the witness data (the second part of a transaction, which is separated from the transaction data):
